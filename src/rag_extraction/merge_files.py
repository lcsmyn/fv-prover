...
if __name__ == '__main__':
    ...
    fname_total = 'dataset_rag_lemmas_total.json'

    with open('dataset_rag_lemmas_['+sys.argv[1:][0]+').json', 'r') as f1:
        merged_list = json.load(f1)
    for index in range(1, len(sys.argv[1:])):
        with open('dataset_rag_lemmas_['+sys.argv[1:][index]+').json', 'r') as f2:
            list_data2 = json.load(f2)
        # Concatenate the lists
        merged_list = merged_list + list_data2


    with open(fname_total, 'w') as f:
